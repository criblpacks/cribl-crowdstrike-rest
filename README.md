...
## Deployment
After installing the Pack, you must perfom the following:
...
2. Obtain a ```Client ID``` and ```Client Secret``` from your Crowdstrike Administrator. These credentials must have ```read``` access to the Hosts, Vulnerabilities, and Alerts API endpionts. 
    ...
    "conf": {
      "discovery": {
        ...
        "discoverRequestParams": [
          ...
          {
            "name": "filter",
            "value": "`updated_timestamp:>'${new Date(earliest*1000*300 || Date.now()-(60*60*1000*24*1)).toISOString()}'`"
          }
        ],
